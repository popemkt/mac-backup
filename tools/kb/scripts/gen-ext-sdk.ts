 ...
 * `src/ext-sdk/surface.ts` via `tsc --emitDeclarationOnly`.
 *
 * Run: `bun tools/kb/scripts/gen-ext-sdk.ts`
 * Writes: `src/ext-sdk/sdk-dts.text.ts`
 */
...
const SURFACE = join(KB_ROOT, "src/ext-sdk/surface.ts");
const OUT_MODULE = join(KB_ROOT, "src/ext-sdk/sdk-dts.text.ts");
...
export async function generateExtSdkDts(
  ...
  try {
    ...
    const dts = [
      ...
      ` * kb ${version}`,
      " * Regenerate: bun tools/kb/scripts/gen-ext-sdk.ts",
...
if (import.meta.main) {
  ...
  console.log(`wrote src/ext-sdk/sdk-dts.text.ts (kb ${version})`);
}
