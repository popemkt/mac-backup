 ...
 * the invoke boundary. `invalid_move` / `forbidden` are preserved as receipt
 * codes (same as ResolveError) even though they sit outside FailureCodeSchema.
 */
...
/** Receipt `code` may be a FailureCode or a ResolveError passthrough. */
export function receiptCodeOf(
  err: DomainError | ResolveError,
): FailureCode | ResolveError["code"] {
  return err.code;
}
