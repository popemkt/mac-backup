...
export function edgePath(
  ...
  const c1x = a.x + (edge.fromSide === "left" ? -dx : edge.fromSide === "right" || !edge.fromSide ? dx : 0);
  const c1y =
    a.y +
    (edge.fromSide === "top" ? -dx : edge.fromSide === "bottom" ? dx : 0);
  const c2x =
    b.x +
    (edge.toSide === "right" ? dx : edge.toSide === "left" || !edge.toSide ? -dx : 0);
  const c2y =
    b.y + (edge.toSide === "top" ? -dx : edge.toSide === "bottom" ? dx : 0);
