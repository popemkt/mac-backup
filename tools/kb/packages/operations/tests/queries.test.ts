...
import { mapBacklinks, mapFieldList, mapSearch, mapTagList } from "../src/map.ts";
...
describe("foundation/query/queries", () => {
  ...
  test("surface mappers use foundation queries without semantic drift", () => {
    expect((mapFieldList().input as { query: string }).query).toBe(LIST_FIELDS_QUERY);
    expect((mapTagList().input as { query: string }).query).toBe(LIST_TAGS_QUERY);
