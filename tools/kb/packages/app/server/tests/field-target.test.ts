...
describe("sys.f.onto.include — targetTag → sys.tag", () => {
  test("the seed really declares sys.tag as the target, via targetTag", () => {
    ...
    expect(present(field, "expected field").props[SYSTEM_IDS.fieldTypeField]).toEqual([
      fieldTypeValue("ref"),
    ]);
  });
  ...
  test("resolves to every supertag in the graph — never the empty set", () => {
    ...
    expect(present(allowed, "expected allowed").size).toBeGreaterThan(0);
    expect([...present(allowed, "expected allowed")].toSorted()).toEqual(seededTagIds);
  });
  ...
  test("a kind is not an instance of itself, and fields are not tags", () => {
    const allowed = present(
      ...
      "expected allowedRefIdsOf(seedMap.get(SYSTEM_IDS.ontoIncludeField), seedMap)",
    );
  ...
  test("a user's own supertag joins the same set with no extra rule", () => {
    ...
    const allowed = present(
      allowedRefIdsOf(seedMap.get(SYSTEM_IDS.ontoIncludeField), nodes),
      "expected allowedRefIdsOf(seedMap.get(SYSTEM_IDS.ontoIncludeField), nodes)",
    );
...
describe("sys.f.targetQuery — the general form", () => {
  test("sys.f.onto.extends resolves #ontology nodes through its seeded query", () => {
    ...
    expect([...present(allowed, "expected allowed")]).toEqual(["o.infra"]);
  });
  ...
  test("wins over targetTag when a field declares both", () => {
    ...
    expect([...present(allowed, "expected allowed")]).toEqual([SYSTEM_IDS.typeField]);
    expect(present(allowed, "expected allowed").has(SYSTEM_IDS.ontologyTag)).toBe(false);
  });
