    ...
    <div className="mx-auto max-w-3xl px-4 py-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="text-ui font-medium text-foreground/80">Ontologies</h2>
          <p className="mt-0.5 max-w-md text-meta text-foreground/40">
        ...
        <button
          type="button"
          disabled={busy}
          className="shrink-0 rounded-md border border-foreground/10 px-3 py-1.5 text-meta text-foreground/70 hover:bg-foreground/5 disabled:opacity-50"
          onClick={() => void onNew()}
        >
          New ontology
        </button>
      ...
      {items.length === 0 ? (
        <p className="text-ui text-foreground/40">No ontologies yet.</p>
      ) : (
        <div className="flex flex-col gap-0.5">
          {items.map((o) => (
            ...
            >
              <span aria-hidden className="shrink-0 text-label text-foreground/35">
                ⬡
              </span>
              <span className="min-w-0 flex-1 truncate text-ui text-foreground/80">{o.label}</span>
              <span className="shrink-0 font-mono text-caption text-foreground/25">{o.id}</span>
            </button>
          ))}
        </div>
      )}
