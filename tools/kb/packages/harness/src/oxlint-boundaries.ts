...
export function generateBoundaryOverrides(
  ...
  for (const p of pkgs) {
    ...
    if (layer && scope && p.manifest.name) {
  ...
  for (const p of [...pkgByDir.values()].sort((a, b) => a.dir.localeCompare(b.dir))) {
    ...
    for (const target of [...pkgByDir.values()].sort((a, b) => a.dir.localeCompare(b.dir))) {
    ...
    if (forbidden.length > 0) {
      const names = forbidden.sort().join("|");
      overrides.push({
