...
- `work` Orca now: `Orca 3173 127.0.0.1:6768 (LISTEN)` + `127.0.0.1:56702` — **loopback only**, NOT `*:6768`.
- `nc -vz 100.114.213.27 6768` → `Connection refused` (tailnet IP unreachable).
- `curl http://127.0.0.1:6768/` → `200 Orca Web` (loopback works).
...
- After fixing reachability, generate fresh `orca://pair?code=...` on `work` with **Pair this** screen held open, then pair from Termux. The `aa1e...` tokens are stale — discard.
