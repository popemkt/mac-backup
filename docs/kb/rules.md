...
- **current** — Closed by wave 2026-09-09 (g8), on top of 9d8e111's declaration. KbNodeSchema.order is optionalKey(NonEmptyString) — absence is the only way to be unranked — and the migration state is the type: migrateOrderKeys returns RankedNode[], with rankOf/isRanked the one way to ask. The DST harness's undefined-rank filter is gone because there are none. The wire contract keeps order optional on purpose, and its two UI readers (restoreInvocations in actions/mutations.ts, forestRootIds in lib/graph-view.ts) ask through rankOf as well, so no call site compares an order against undefined.
