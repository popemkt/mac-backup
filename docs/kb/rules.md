...
- **current** — invoke (session/runtime.ts) applies a write to the replica locally and pushes it on the ordered lane; it never merges the POST receipt. The replica is written only by socket tx frames, which live.ts applies through mergeRemoteUpserts as they arrive, and mergeRemoteUpserts keeps only a pending text buffer. Each frame carries the whole node as the server had it after that one write, so the frame for write k erases local writes k+1..n until their own frames land, and a POST resolving says nothing about whether its frame has been merged yet. SubscriptionHub's claim that an optimistic apply is idempotent under its confirming frame holds only while one write is in flight. palette.e2e.ts works around it by waiting for the test-render rev marker (data-kb-rev on the root element) to reach the server head after its write.
