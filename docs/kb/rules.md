...
| Effect v4 idiom | tools/kb/AGENTS.md#effect | tools/kb | Read the shipped Effect AGENTS.md before writing Effect code; the v4 non-negotiables listed there are not optional and v3 memory is wrong. | harness | — |
...
| Lazy 3D chunk | tools/kb/harness/src/constraints.ts | tools/kb packages/app/ui | three reaches the page only through a dynamic import(): nothing the UI entry loads eagerly imports it, directly or through the scene kit's GPU modules, so each 3D view is its own chunk and no surface lists which of its files may import three. | harness | — |
...
| Mutation score is advisory | tools/kb/DESIGN.md#testing-doctrine | tools/kb | The kill score is a sensor a human reads to find a missing test; it never blocks a merge, and kb's own weekly run declares itself non-reproducible. | harness | — |
