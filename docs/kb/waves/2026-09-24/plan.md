...
| g | graph polish: one shared scene module (bloom, fog, starfield) used by lab and force3d, with link particles and camera fly-to in the graph only; 2D curved edges, label halos, hover fade | e, f2 | merged `1ddb4f9` (grok: approve after 4 rounds) |
